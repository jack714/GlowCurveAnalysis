                  ...
                  std::vector<int>& inflectPnt )
{
    ...
    // Original Value = 10
    double low_threshold = (avg < 10)? avg : 10;
    ...
    for( int i = 0 ; i < int( maxima.size( ) ) ; i++ )
    {
        ...
        //y[maxima[i]] < y[maxima[i] - near_point_height]
        if( y[maxima[i]] < low_threshold )
        {
            maxima.erase( maxima.begin( ) + i );
            i--;
            continue;
                  ...
                  std::string output_dir)
{
    ...
    {
        if( yTemp[j] < 0.0 || (!maxima.empty() && j > maxima.back()))
        {
            yTemp[j] = 0.0;
        }
    }
